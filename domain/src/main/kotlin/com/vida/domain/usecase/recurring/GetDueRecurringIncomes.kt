 ...
 */
class GetDueRecurringIncomes(private val repo: RecurringIncomeRepository) {
    operator fun invoke(asOf: LocalDate = LocalDate.now()): Flow<List<RecurringIncome>> =
        repo.getAll().map { all ->
            all.filter { r -> r.isActive && isDue(r, asOf) }
        }

    private fun isDue(r: RecurringIncome, asOf: LocalDate): Boolean {
    ...
    private fun nextDueDate(r: RecurringIncome): LocalDate? {

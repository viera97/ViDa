...
class WalletMapperTest {
    ...
    fun `round trip preserves all fields`() {
        val wallet = Wallet(currency = "CUP", balance = Money.of("0.00", Currency.CUP))
    ...
    fun `all currencies round trip`() {
        for (currency in Currency.values()) {
            val wallet = Wallet(currency = currency.code, balance = Money.of("0.00", currency))
            val entity = mapper.toEntity(wallet)
            val roundTrip = mapper.toDomain(entity)
            assertEquals(wallet, roundTrip)
        }
    ...
    fun `id is preserved across round trip`() {
        val wallet = Wallet(id = 7L, currency = "MLC")
        val entity = mapper.toEntity(wallet)
    ...
    fun `name maps entity to domain`() {
        val entity = mapper.toEntity(Wallet(currency = "CUP", balance = Money.of("0.00", Currency.CUP)))
    ...
    fun `name maps domain to entity`() {
        val wallet = Wallet(currency = "USD", name = "Billetera USD", balance = Money.of("0.00", Currency.USD))
        val entity = mapper.toEntity(wallet)
